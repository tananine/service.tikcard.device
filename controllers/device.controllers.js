...
const getRedirect = async (req, res, next) => {
  ...
  try {
    ...
    if (setAccountAction) {
      return res.status(200).json({ setAccountAction: true, message: 'ตั้งค่า Device เชื่อม Account' });
    }
...
const activate = (req, res, next) => {
  ...
  db.Scanner.findByPk(scannerId, {
    ...
    return db.Device.update({ accountId: accountId }, { where: { id: scanner.deviceId } })
  }).then(update => {
    if (update[0]) {
